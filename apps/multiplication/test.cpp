...
int main(int argc, char **argv) {
    ...
    }
    {
        std::map<std::string, Halide::Internal::Function> funcs = Halide::Internal::find_transitive_calls((result_).function());
         ...
         AB.update(0).split(rv, rvo , rvi ,4);
         As.reorder(ii,io,ji,jo,z);
         AB.update(0).reorder(rvi,rvo,j,ii,io);
         result_.reorder(ii,ji,io,jo);
         As.parallel(z);
         AB.update(0).parallel(io);
         result_.parallel(jo);
         As.vectorize(ji);
         AB.update(0).vectorize(ii);
         result_.vectorize(ii);
         AB.update(0).unroll(rvi);
         result_.unroll(ji);
         As.compute_at(AB,ii);
         AB.compute_root();        
